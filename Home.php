<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>AsesomeShop</title>
    <link rel="stylesheet" href="https://stackpath.bootstrapcdn.com/bootstrap/4.5.0/css/bootstrap.min.css" integrity="sha384-9aIt2nRpC12Uk9gS9baDl411NQApFmC26EwAOH8WgZl5MYYxFfc+NcPb1dKGj7Sk" crossorigin="anonymous">
    <script src='https://kit.fontawesome.com/a076d05399.js'></script>
</head>
<style>
    .head {
    float: left;
    margin-left: 25px;
    margin-top: 1pc;
    }
    .head1 {
    float: right;
    margin-right: 25px;
    margin-top: 1pc;
    }
    .shop {
        color: green;
    }
    .img1 {
        min-width: 100%;
        height: 15cm;
        margin-top: 1px;
        position: absolute;
        width: 1024px;
        right: 0;
        right: 0;
    }
    .text_absolute {
        position: absolute;
        
        right: 30%;
    }
    .font {
        font-family: Arial, "Helvetica Neue", Helvetica, sans-serif;
        size: 40px;
    }
    .input-icons { 
            width: 100%; 
            margin-bottom: 10px; 
    } 
    .input-field { 
            width: 100%; 
            padding: 10px; 
            text-align: left;
    } 
    .box1 {
        background-origin: padding-box;
        background-color: lightcoral;
        width: 350px;
        height: 170px;
        border: 2px solid black;
        padding: 20px;
        display: inline-flex;
        
    }
    .box2 {
        background-origin: padding-box;
        background-color: lightsalmon;
        width: 350px;
        height: 170px;
        border: 2px solid black;
        padding: 20px;
        display: inline-flex;
    }
    .box3 {
        background-origin: padding-box;
        background-color: plum;
        width: 350px;
        height: 170px;
        border: 2px solid black;
        padding: 20px;
        display: inline-flex;
    }
    .dis_box {
        display: grid;
        grid-template-columns: auto auto auto;
        margin-left: 100px;
        
    }
    .bx1 {
        border: 5px;
        box-shadow: 5px lightcoral;
      
    }
    .last_container {
        margin-left: 100px;
        display: grid;
        grid-template-columns: auto auto auto;
    }
    .price_box {
        background-origin: padding-box;
        border: solid black 2px;
        width: 1100px;
        height: 400px;
        grid-template-columns: auto auto auto;
        display: grid;
    }
    .comment {
        border: solid black 2px;
        width: 1100px;
        height: 200px;
    }
    .dis_btn_price {
        display: inline-flex;
    }
    .discount {
        background-color: rgb(238, 184, 84);
        border-radius: 10px;
        width: 50px;
        height: 30px;
        border: none;
        margin-top: 7px;
        margin-left: 8cm;
    }
    .img_head {
        display: inline-flex;
        margin-top: 10px;
    }
    .text_head {
        background-color: rgb(235, 227, 227);
        width: 525px;
        height: 525px;
        text-align: center;
    }
    .small_img {
        width: 70px;
        height: 70px;
    }
    .dis_small_img {
        display: inline-flex; 
        margin-top: 7cm; 
        margin-left: -8cm;
    }
    .cloth {
        width: 150px;
        height: 130px;
        margin-top: 30px;
        margin-left: 150px;
    }
    fieldset {
        width: 1100px; height: 200px; border: solid 2px black;
        color: black;
        background-color: rgb(186, 212, 221);
    }
   

</style>
<body>
    <div>
        <div class="head">
            <h1>Awesome<span class="shop">Shop</span></h1>
        </div>
        <div> 
            <div class="head1" style="box-shadow: 3px 3px black;"><input type="button" value="Join"></div>
            <div class="head1"><i class="fa fa-question-circle" >Need Helps</i></div>
        </div>
    </div>

    <div class="img_head">
        <div><img src="b7.jpg" alt="" style="width: 525px; height: 525px;"></div> 
        <div class="text_head">
            <p style="font-size:xx-large; color: black;font-family: Arial, Helvetica, sans-serif;"><b>Shop your designer dresses</b></p><br>
            <p style="font-size: large; color: black;"><b> Ready to wear dresses tailored for you from online. Hurry up while stock lasts.</b></p><br>
                <div class="input-icons"> 
                    <i class="fa fa-search icon"> 
                  </i> 
                    <input class="input-field" 
                           type="text" 
                           style="border-radius: 15px; width: 450px; height: 40px; border: 0px;"
                           placeholder="Search your product here"> 
                </div> 
        </div>
        <div><img src="b5.jpg" alt="" style="width: 525px; height: 525px;"></div> 
    </div>
    
    <div style="margin-top: 2cm;" class="dis_box">
            <div class="box1" >
                <div>
                    <p><b>Coupon Saving</b><br>Up to 60% of everyday essentails</p>
                    <input type="button"  class="bx1" value="Shop Coupons"; style="box-shadow: 3px 4px black;">
                </div>
                <div>
                    <img src="Scissor3.png" alt="no" style="width: 60px;height: 50px;">
                </div>
            </div>

            <div class="box2" >
                <div>
                    <p ><b>Free dilivery</b><br>With etablish items</p>
                    <input type="button" value="Dilivery Now"; style="box-shadow: 3px 4px black;">
                </div>
                <div>
                    <img src="tru1.png" alt="no" style="width: 90px;height: 80px;">
                </div>
            </div>

            <div class="box3" >
                <div>
                    <p><b>Gift Vecher</b><br>With personal core items</p>
                    <input type="button" value="Gift Now"; style="box-shadow: 3px 4px black;">
                </div>
                <div>
                    <img src="g2.png" alt="no" style="width: 60px;height: 50px;">
                </div>
            </div>   
    </div><br><br>

    <div class="last_container">
        <div>
            <div>
                <img src="TV1.png" alt="no" style="width: 50px;height: 50px;" alt=""> Electronics
            </div><br>
            <div>
                <img src="bag1.png" alt="no" style="width: 50px;height: 50px;" alt=""> Hand bag
            </div><br>
            <div>
                <img src="b1.png" alt="no" style="width: 50px;height: 50px;" alt=""> Wallet
            </div><br>
            <div>
                <img src="s.webp" alt="no" style="width: 50px;height: 50px;" alt=""> Clothes
            </div>
        </div>
        <div>
            <div>
                <div class="price_box">
                    <div>
                        <input type="button" class="discount" value="20%"><br><br>
                        <img src="sh1.jpg" alt="" class="cloth">
                    </div>
                    <div class="dis_small_img">
                        <img src="sh1.jpg" alt="" class="small_img">
                        <img src="sh1.jpg" alt="" class="small_img">
                        <img src="sh1.jpg" alt="" class="small_img">
                    </div>
                    <div style="margin-left: 2cm;">
                        <p>Black T-Shirt - Nike<span style="color: red;">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<del>$15</del></span> <span style="color: green;">$12</span></p>
                        <p>Nike Womens Pink Half Zip Lightweight Hooded Shirt Top Athletio <br>Stretch Size M</p>
                        <div style="margin-top: 4.5cm;">
                            <button style="border: 2px; box-shadow: 2px 3px black;"><i class="fas fa-shopping-cart"></i> Card</button>
                        </div>
                        <div style="display: grid; margin-top: 0.5cm; grid-template-columns: auto auto;">
                            <div>
                                <input type="button" value="Male T-Shirt" style="border: none; background-color: lightgray; border-radius: 10px; height: 30px;">
                            </div>
                            <div>
                                <input type="button" value="Single Color" style="border: none; background-color: lightgray; border-radius: 10px; height: 30px;">
                            </div>
                        </div>
                    </div>
                </div>
            </div><br><br>

            <div>
                <p><b>Comment</b></p>
            </div>
            <div>
                <div class="comment">
                    <div style="display: inline-flex; margin-top: 10px; margin-left: 10px;">
                        <div><i class="fas fa-user-circle" style="font-size: 3em;"></i></div>
                        <div><p><span style="color: blue;">Anonymous </span><br>Today 4:21pm - 12.06.2020</p></div>
                    </div>
                    <div>
                        <p>Lorem Ipsum is simply dummy text of the printing and typesetting industry Lorem Ipsum has been the industry's standard dummy text ever since the 1500s, when on unknown printer took a galley of type and scramble it to make a type spaceimen book. It has survived not only five centuries, but also the leap into electronic typesetting, remaining esssentially enchanged.</p>
                    </div>
                </div>
                <div class="comment">
                    <div style="display: inline-flex; margin-top: 10px; margin-left: 10px;">
                        <div><i class="fas fa-user-circle" style="font-size: 3em;"></i></div>
                        <div><p><span style="color: blue;">Anonymous </span><br>Today 4:21pm - 12.06.2020</p></div>
                    </div>
                    <div>
                        <p>Lorem Ipsum is simply dummy text of the printing and typesetting industry Lorem Ipsum has been the industry's standard dummy text ever since the 1500s, when on unknown printer took a galley of type and scramble it to make a type spaceimen book. It has survived not only five centuries, but also the leap into electronic typesetting, remaining esssentially enchanged.</p>
                    </div>
                </div>
            </div><br>
            <fieldset><legend>Write your review here</legend></fieldset><br>
            <div style="margin-left: 935px;">
                <button type="submit" style="background-color: green; color: white; box-shadow: 4px 4px black;"><i class="fas fa-reply"></i>Send</button>
                <button type="submit" style="background-color: rgb(253, 36, 36); color: white; box-shadow: 4px 4px black;"><i class="fas fa-times"></i>Discard</button> <br>
            </div>
        </div><br>
      
    </div>
</body>
</html>


